...
function HealthBadge({ display, status }) {
...
function FeeEstimate({ destChainId }) {
  ...
  const feeDisplay = parseFloat(formatUnits(feeRaw, 10)).toFixed(4);
...
export function BorrowPanel() {
  ...
  // Oracle DOT price for collateral USD display
  ...
  const collateralUsd = useMemo(() => {
    ...
    // wdotValueInUsd = wdot * dotPrice / 1e10 (wdot decimals) — result in WAD
    const usdWad = (totalWdot * dotPriceWad) / (10n ** 10n);
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', textAlign: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: '16px', fontWeight: 'bold' }}>
              {parseFloat(collateralFormatted).toFixed(4)} WDOT
            </div>
          ...
          <div>
            ...
            <div style={{ marginTop: '4px' }}>
              <HealthBadge display={hfDisplay} status={hfStatus} />
            </div>
          </div>
        </div>
      </div>
      ...
      {!isActive && (
        <div style={cardStyle}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            />
            <button
              onClick={() => setUsdcAmount(formatUnits(maxUsdc, USDC_DECIMALS))}
              style={{
                position: 'absolute',
                right: '8px',
                top: '50%',
          ...
          {previewHf && (
            <div style={{ ...hintStyle, display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <HealthBadge display={previewHf.display} status={previewHf.status} />
            </div>
          )}
          ...
          <div style={{
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
              <input
                type="checkbox"
                checked={remitEnabled}
                onChange={(e) => setRemitEnabled(e.target.checked)}
                style={{ width: '18px', height: '18px', accentColor: '#e91e8c' }}
              />
              <span style={{ fontSize: '14px', color: '#ccc' }}>
                Send cross-chain (remittance)
              </span>
            </label>
          </div>
            ...
            <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div>
                ...
                {recipient && !isAddress(recipient) && (
                  <div style={{ color: '#A32D2D', fontSize: '12px', marginTop: '4px' }}>
                    Invalid address
                  </div>
                )}
              </div>
          ...
          <button
            ...
            disabled={!borrowValid || !remitValid || borrowLoading}
          ...
          {borrowTx && (
            <div style={{ fontSize: '13px', marginTop: '8px', color: '#22c55e' }}>
              Success:{' '}
              <a href={SUBSCAN_TX + borrowTx} target="_blank" rel="noopener noreferrer" style={{ color: '#22c55e' }}>
                View on Subscan
              </a>
            </div>
          )}
      ...
      {isActive && (
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '16px', textAlign: 'center' }}>
            <div>
              <div style={{ color: '#888', fontSize: '13px' }}>Principal</div>
              <div style={{ fontWeight: 'bold' }}>{parseFloat(repPrincipal).toFixed(2)} USDC</div>
            </div>
            <div>
              <div style={{ color: '#888', fontSize: '13px' }}>Interest</div>
              <div style={{ fontWeight: 'bold' }}>{parseFloat(repInterest).toFixed(4)} USDC</div>
            </div>
            <div>
              <div style={{ color: '#888', fontSize: '13px' }}>Total Due</div>
              <div style={{ fontWeight: 'bold', color: '#e91e8c' }}>{parseFloat(repTotal).toFixed(4)} USDC</div>
            </div>
          </div>
          ...
          {repayTx && (
            <div style={{ fontSize: '13px', marginTop: '8px', color: '#22c55e' }}>
              Repaid:{' '}
              <a href={SUBSCAN_TX + repayTx} target="_blank" rel="noopener noreferrer" style={{ color: '#22c55e' }}>
                View on Subscan
              </a>
            </div>
          )}
