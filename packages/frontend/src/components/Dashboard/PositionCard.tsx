...
import { usePosition, useHealthFactor, useRepaymentAmount, useRepay, type HealthStatus } from '../../hooks/useLendingPool';
...
const btnBase: React.CSSProperties = { width: '100%', padding: '14px', borderRadius: '8px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer', marginTop: '16px' };
...
function HealthBar({ display, status, raw }: { display: string; status: HealthStatus; raw: bigint }) {
  ...
  return (
    <div>
      ...
      <div style={{ width: '100%', height: '8px', background: '#0d0d1a', borderRadius: '4px', overflow: 'hidden' }}>
        <div style={{ width: `${fillPct}%`, height: '100%', background: color, borderRadius: '4px', transition: 'width 0.3s ease' }} />
      </div>
    </div>
  );
}
    ...
    return (
      <div style={cardStyle}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: '24px', color: '#22c55e', fontWeight: 'bold', marginBottom: '12px' }}>Position Closed</div>
          <div style={{ color: '#aaa', fontSize: '14px', marginBottom: '8px' }}>{returnedWdot} WDOT returned to available balance</div>
          <a href={EXPLORER_TX + repayTx} target="_blank" rel="noopener noreferrer" style={{ color: '#e91e8c', fontSize: '13px' }}>View on Explorer</a>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={cardStyle}>
        ...
        <div style={{ textAlign: 'center', color: '#888', padding: '24px 0' }}>
          ...
          <div style={{ fontSize: '14px' }}>Deposit collateral and borrow USDC to get started.</div>
        </div>
      </div>
    );
  }
    ...
    <div style={cardStyle}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <span style={{ fontWeight: 'bold' }}>{parseFloat(formatUnits(collateralWdot, WDOT_DECIMALS)).toFixed(4)} WDOT</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ color: '#888' }}>Borrowed</span>
          <span style={{ fontWeight: 'bold' }}>${parseFloat(repPrincipal).toFixed(2)} USDC</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ color: '#888' }}>Accrued interest</span>
          <span style={{ fontWeight: 'bold' }}>${parseFloat(repInterest).toFixed(4)} USDC</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #2a2a4e', paddingTop: '12px' }}>
          ...
          <span style={{ fontWeight: 'bold', color: '#e91e8c', fontSize: '18px' }}>${parseFloat(repTotal).toFixed(4)} USDC</span>
        </div>
