...
export function LiquidationPanel({ onAddCollateral, onRepay }: LiquidationPanelProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '24px', marginBottom: '16px', fontSize: '14px' }}>
        <div>
          <span style={{ color: '#888' }}>Current health factor: </span>
          <span style={{ fontWeight: 'bold', color: HF_COLORS.danger, fontSize: '16px' }}>{hfDisplay}</span>
        </div>
        <div>
      ...
      <div style={{ display: 'flex', gap: '12px' }}>
        <button onClick={onAddCollateral} style={{
          flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid #A32D2D',
          background: 'transparent', color: '#ff6b6b', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer',
        }}>
          Add Collateral
        </button>
        <button onClick={onRepay} style={{
          flex: 1, padding: '12px', borderRadius: '8px', border: 'none',
          background: '#A32D2D', color: '#ffffff', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer',
        }}>
          Repay Now
        </button>
      </div>
